...
/**
 * Full export: Notion → .typ → .pdf (if Typst is installed).
 *
 * Writes .typ before attempting PDF compilation so the source is always
 * available even when Typst is not installed.
 */
export async function exportTypstPdf(
  ...
  if (doc.validation.errors.length > 0) {
    console.warn(
      `[PDF] Document has ${doc.validation.errors.length} validation error(s) — exporting anyway.`
    );
  }

  // Download image assets to outDir/assets/docs/{docId}/
  // copyDocumentAssets sets asset.outputPath; the renderer uses the filename segment.
  ...
  // Write .typ first — always, before checking for Typst binary
  ...
  if (!checkTypst()) {
    return { docId: doc.meta.docId, title: doc.meta.title, typPath, pdfPath: null, lineCount };
  }
